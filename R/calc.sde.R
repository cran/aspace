...
function(id=1, filename="SDE_Output.txt", calccentre=FALSE, useWMC=FALSE, centre.xy=centre, destmat=activities, titletxt="Title", verbose=FALSE, plot=TRUE, calcSDxy=TRUE, plotSDEaxes=TRUE, plotdest=TRUE, plotcentroid=TRUE, plotSDxy=TRUE, weightpoints=FALSE, weights=wts, jpeg=FALSE) {
  ...
  #  DATE:      27 MARCH 2006
  #  CALLS:     distances(), calcsdd(), ellipse3(), atan.d(), sin.d(), cos.d()
  #  NEEDS:     
  ...
  if(tantheta < 0) {
    theta <- 90 - atan.d(abs(tantheta))
  }
  else {
    theta <- atan.d(tantheta)
  }
    
  # COMPUTE OTHER TRIGONOMETRIC VALUES
  sintheta <- sin.d(theta)
  costheta <- cos.d(theta)
  ...
  if(plot) {
    ellipse3(centre.xy[1], centre.xy[2], sigmax, sigmay, as.radians(theta) )
  }
  title(paste("Graphic Results:", titletxt, sep=""))
  coordsSDE <- ellipse3(centre.xy[1], centre.xy[2], sigmax, sigmay, as.radians(theta), pointsonly=TRUE)
  ...
  if(plotSDEaxes) {    
    ...
    xprime <- centre.xy[1] + ( sigmay * cos.d(90-theta) )
    yprime <- centre.xy[2] + ( sigmay * sin.d(90-theta) )
    segments(centre.xy[1], centre.xy[2], xprime, yprime, col=3)
    
    # PLOT HALF-AXES (SIGMA-X) FOR ELLIPSE IN BLUE
    xprime <- centre.xy[1] + ( sigmax * cos.d(theta) )
    yprime <- centre.xy[2] - ( sigmax * sin.d(theta) )
