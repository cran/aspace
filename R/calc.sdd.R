...
function(id=1, filename="SDD_Output.txt", centre.xy=centre, calccentre=TRUE, useWMC=FALSE, weightpoints=FALSE, weights=wts, destmat=activities, verbose=FALSE, plot=TRUE, plothv=TRUE, plotdest=TRUE, plotcenter=TRUE, box=TRUE) {
  ...
  #  DATE:      27 MARCH 2006
  #  CALLS:     distances(), ellipse(), as.radians()
  #  NEEDS:     
  ...
  if(length(dist.v) >= 3) {
    ...
    if(plot) {
      ...
      ellipse3(centre.xy[1], centre.xy[2], SDD, SDD, as.radians(0), col=6)
    ...
    coordsSDD <- ellipse3(centre.xy[1], centre.xy[2], SDD, SDD, as.radians(0), col=6, pointsonly=TRUE)
